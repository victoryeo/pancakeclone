...
contract PancakePair is IPancakePair, PancakeERC20 {
    ...
    function token0() external override(IPancakePair) view returns (address) {
      return _token0;
    }

    function token1() external override(IPancakePair) view returns (address) {
      return _token1;
    }

    function price0CumulativeLast() external override(IPancakePair) view returns (uint) {
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IPancakeFactory(_factory).feeTo();
        feeOn = feeTo != address(0);
        //uint _kLast = _kLast; // gas savings
        if (feeOn) {
            if (_kLast != 0) {
                uint rootK = Math.sqrt(uint(_reserve0).mul(_reserve1));
                uint rootKLast = Math.sqrt(_kLast);
                if (rootK > rootKLast) {
                    uint numerator = _totalSupply * (rootK.sub(rootKLast));
